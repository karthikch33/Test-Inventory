...
const Card = ({ title, description, icon, link, btnText }) => {
  const cardStyle = {
    backgroundColor: '#fff',
    borderRadius: '10px',
  ...
  return (
    ...
    <Link to={link} style={{ textDecoration: 'none' }}>
      <div className="card">
        ...
        <button className="btn btn-priamry primary-button">{btnText}</button>
        <p className='card-description secondary-link '>{description}</p>
      </div>
    </Link>
  );
};
