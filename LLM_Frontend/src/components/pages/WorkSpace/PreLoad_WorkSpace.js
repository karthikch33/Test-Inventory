...
import { getAllFilesByProjectIdSlice, getPostLoadReportSlice, getPreloadTableSlice, loadPreloadTablesSlice } from '../../features/WorkSpace/workSpaceSlice'
import FormItem from 'antd/es/form/FormItem'

const PreLoad_WorkSpace = () => {
    const {Search} = Input
    ...
    const formik = useFormik({
     ...
     const handleProjectChange = (value) => {  
            ...
            .then((response) => {  
                ...
                formik.setFieldValue('source_files',filteredSourceFiles);
            })
    };
    ...
    const handleSourceChange = (value)=>{
        formik?.setFieldValue('selected_source',value);
    }
    ...
    const handleSubmit = ()=>{
        ...
        const data = {
            ...
            preload_id : formik?.values?.selected_source,
            postload_id : formik?.values?.selected_preload
        }

       dispatch(getPostLoadReportSlice(data))
        .then((response)=>{
  ...
  return (
    ...
    <div>
        ...
        <Form.Item>
        <label style={{ color: "skyblue", fontSize: "20px",marginRight:"20px",marginLeft:"10px" }}>PreLoad</label> 
        </Form.Item>
        ...
        <Form.Item label="Source" className="mb-0" style={{ minWidth: 250, maxWidth: 250}}>
            <Select
            style={{width:160}}
            onChange={handleSourceChange}  
            value={formik?.values?.selected_source || undefined}  
            dropdownStyle={{ overflowY: 'auto' }}  
            >  
            {formik?.values?.source_files && formik?.values?.source_files?.map((source_file) => (  
                <Option key={source_file?.table_id} value={source_file?.table_id}>  
        ...
        {/* <Form.Item>
         <Search placeholder="Search Here.."
            ...
            style={{width:"270px"}} /> 
             ...
             <div className='row' style={{ height: "75vh",width:"100vw",marginTop:"10px",overflow : "auto"}}>
            <Table 
...
export default PreLoad_WorkSpace